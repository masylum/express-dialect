...
module.exports = function (options, callback) {
  ...
  Dialect.store({store: options.store, database: options.database}, function (error, store) {
    ...
    dialect.regenerateJSON(function () {
      var t = options.helper || 't';

      function tfunction(str) {
        return dialect.getTranslation(str);
      }

      // Expose helper t()
      express_dialect.helpers = function (req, res, next) {
      ...
      express_dialect.dynamic_helpers[t] = function (req, res) { 
        var locales = utils.intersect(req.locales, dialect.config('locales'));
      ...
      if (options.app) {
        options.app.dynamicHelpers(express_dialect.dynamic_helpers);
      };
